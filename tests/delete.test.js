...
beforeEach(async () => {
  ...
  tempHome = await mkdtemp(join(tmpdir(), 'taskr-delete-test-'));
});

// --- Happy path ---

test('delete: removes the targeted task and prints confirmation', async () => {
  const seeded = await addTaskAndGetId(tempHome, 'Buy milk');
  const taskId = seeded.id; // numeric from storage

  const { exitCode, stdout } = await runTaskr(['delete', String(taskId)], tempHome);
...
test('delete: correctly matches numeric storage id given as string argument', async () => {
  ...
  const { exitCode, stdout } = await runTaskr(['delete', String(numericId)], tempHome);
...
test('delete: exits with code 1 and prints usage when no id is provided', async () => {
  const { exitCode, stderr } = await runTaskr(['delete'], tempHome);
...
test('delete: exits with code 1 and prints "not found" when id does not exist', async () => {
  ...
  const { exitCode, stderr } = await runTaskr(['delete', nonExistentId], tempHome);
...
test('delete: exits with code 1 when storage file does not exist (cold start)', async () => {
  ...
  const { exitCode, stderr } = await runTaskr(['delete', '1234567890001'], tempHome);
...
test('delete: removes only the targeted task; sibling tasks remain untouched', async () => {
  ...
  const { exitCode, stdout } = await runTaskr(['delete', String(targetId)], tempHome);
...
test('delete: deleting the last task leaves storage with an empty array', async () => {
  ...
  const { exitCode } = await runTaskr(['delete', String(numericId)], tempHome);
  assert.equal(exitCode, 0);
...
test('delete: deletes only the task with the matching id, even when titles are duplicated', async () => {
  ...
  const { exitCode } = await runTaskr(['delete', String(firstId)], tempHome);
  assert.equal(exitCode, 0);
...
test('delete: task added via add command can be deleted and disappears from list', async () => {
  const seeded = await addTaskAndGetId(tempHome, 'Integration delete task');
  const taskId = seeded.id;

  await runTaskr(['delete', String(taskId)], tempHome);

  const { stdout } = await runTaskr(['list'], tempHome);
  assert.ok(
